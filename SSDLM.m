...
%     while (-fzk'*dk < sigma*tk*norm(fzk)*norm(dk)^2 & tk>10^(-10))
    while (-fzk'*dk < sigma*tk*norm(dk)^2 & tk>10^(-10))
        tk = rho*tk;
        zk = xk + tk*dk;
        fzk = F(zk);
        nf = nf+1;      
...
%     sk=tk*dk;
    ...
    ak=yk+0.1*sk;
   bk=(fknew'*ak)/(dk'*ak);
   ck=1.1*norm(ak)^2/(sk'*ak);
   qk=(norm(ak)^2*fknew'*dk)/norm(dk'*ak)^2;
 
       dknew=-0.7*fknew+0.7*bk*dk-0.7*ck*((fknew'*sk)/(dk'*ak))*dk-qk*dk+0.7*(fknew'*dk)/(dk'*ak)*ak;
